...
@if( $modules->count() > 0 && $operation == 'modules_lessons' )
<ul class="nav nav-tabs" role="tablist">
  <li class="nav-item">
    <a class="nav-link active" data-toggle="tab" href="#lessons" role="tab">{{getPhrase('lessons')}}</a>
  </li>
  <li class="nav-item">
    <a class="nav-link" data-toggle="tab" href="#modules" role="tab">{{getPhrase('modules')}}</a>
  </li>
</ul>
@endif
	...
	<div class="white-card cs-card mb-4">
		<div class="flow-hidden relative p-3">
			...
			<div class="row">
				...
				<div class="col-sm-6">
					<h5 class="text-green">{{$course->title}}</h5>
					@if( ! empty( $course->short_description ) )
					<p class="course-card-text">{!!$course->short_description!!}</p>
					@endif
					<div class="mt-2"><a href="{{URL_LMS_SHOW_MODULE_LESSONS . $item->slug . '/' . $course->slug}}" class="btn btn-kg btn-course btn-round {{$see_more_class}}">{{getPhrase('see_more')}}</a></div>
				</div>
			</div>
		</div>
	</div>
...
<div class="tab-pane active" ng-controller="singleLessonCtrl" id="lessons" role="tabpanel">
<div class="row mt-4">
	<?php
	$sl_no = 1;
