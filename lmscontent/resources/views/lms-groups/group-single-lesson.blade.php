...
<ol class="breadcrumb mt-3">
	...
	<li class="breadcrumb-item"><a href="{{URL_STUDENT_DASHBOARD_GROUP . $series_details->slug}}">{{getPhrase('Group')}} ({{$series_details->title}})</a></li>
	
	<li class="breadcrumb-item"><strong class="text-green">{{$item->title}}</strong></li>
</ol>
<?php
...
<div class="row mt-4">
    <div class="col-sm-12">
            <div class="video-container">
                ...
                $is_completed = FALSE;
                if ( Auth::check() ) {
                    $getstatus = App\LmsTrack::getStatus( $display_item->id, 'video', $course_id, $module_id );
                ...
                ?>
                @if( $is_completed )
                    <button class="fixed-top-left task-btn" ng-click="mark_as_complete('{{$lesson_id}}', 'video-uncomplete', '{{$course_id}}', '{{$module_id}}', 'group')">
                @else
                    <button class="fixed-top-left task-btn" ng-click="mark_as_complete('{{$lesson_id}}', 'video', '{{$course_id}}', '{{$module_id}}', 'group')">
                @endif
                ...
                <div class="fixed-top-right ">
                    ...
                    if ( Auth::check() ) {
                        if ( is_lesson_piece_completed( $display_item->id, $course_id, $module_id ) ) {
...
<div class="row">
    <div class="col-sm-12">
        <div class="text-container mt-4">
            ...
            $is_completed = FALSE;
            if ( Auth::check() ) {
                $getstatus = App\LmsTrack::getStatus( $display_item->id, 'text', $course_id, $module_id );
                if ( ! empty( $getstatus ) && $getstatus->status == 'completed' ) {
                    $icon_class = 'icon icon-tick-border'; // If it is completed
                    $is_completed = TRUE;
                }
            ...
            ?>
            @if( $is_completed )
                <button class="fixed-top-left task-btn" ng-click="mark_as_complete('{{$lesson_id}}', 'text-uncomplete', '{{$course_id}}', '{{$module_id}}', 'group')">
            @else
                <button class="fixed-top-left task-btn" ng-click="mark_as_complete('{{$lesson_id}}', 'text', '{{$course_id}}', '{{$module_id}}', 'group')">
            @endif
    ...
    <div class="col-sm-12">
        <div class="quiz-container text-center">
            <div class="take-quiz-btn">
                ...
                $icon_class = 'icon icon-tick-double';
                if ( Auth::check() ) {
                    $getstatus = App\LmsTrack::getStatus( $display_item->id, 'quiz', $course_id, $module_id );
                    if ( ! empty( $getstatus ) && $getstatus->status == 'completed' ) {
                        $icon_class = 'icon icon-tick-border'; // If it is completed
                        $is_completed = TRUE;
                    }
                ...
                ?>
                @if( $is_completed )
                    <button class="fixed-top-left task-btn" ng-click="mark_as_complete('{{$lesson_id}}', 'quiz-uncomplete', '{{$course_id}}', '{{$module_id}}', 'group')">
                @else
                    <button class="fixed-top-left task-btn" ng-click="mark_as_complete('{{$lesson_id}}', 'quiz', '{{$course_id}}', '{{$module_id}}', 'group')">
                @endif
                ...
                @else
                    <button class="btn btn-primary-outline btn-min-width btn-top-icn" ng-click="mark_as_complete('{{$lesson_id}}', 'quiz', '{{$course_id}}', '{{$module_id}}', 'group')">{{getPhrase('Lesson Quiz')}}</button>
                @endif
            </div>
        </div>
    </div>
...
<div class="row justify-content-between mt-5 mb-8">
    ...
    <div class="col-md-5">
        ...
        @else
        <button class="btn btn-primary-outline btn-min-width btn-block" ng-click="mark_as_complete('{{$lesson_id}}', 'quiz', '{{$course_id}}', '{{$module_id}}', 'group')">{{getPhrase('Take Notes')}} <i class="fa fa-pencil-square"></i></button>
        @endif
        <!-- Modal -->
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-primary" ng-click="saveNotes({{$display_item->id}}, {{$series_details->id}})">{{getPhrase('Save')}}</button>
                    </div>
                    {!! Form::close() !!}
