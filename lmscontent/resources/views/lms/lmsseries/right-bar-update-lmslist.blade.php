				...
				<div ng-if="savedSeries!=''" class="vertical-scroll" >
								  ...
								  <tr>
									...
									<tbody>
										...
										@if( $record->is_paid == 1 && $record->cost > 0 )
										<td>										
										<select name="is_free[@{{$index}}]" ng-model="is_free">
											<option value="no" ng-selected="@{{savedSeries[$index].is_free == 'no'}}">{{getPhrase('no')}}</option>
											<option value="yes" ng-selected="@{{savedSeries[$index].is_free == 'yes'}}">{{getPhrase('yes')}}</option>
										</select></td>
										@else
