                    ...
                    <div class="col-lg-12">
                        <ol class="breadcrumb mt-2">
							@if(checkRole(getUserGrade(2)))
								<li class="breadcrumb-item"><a href="{{URL_SITE_ISSUES}}">{{ getPhrase('site_issues')}}</a> </li>
							@else
...
{!! Form::open(array('url' => URL_SITE_ISSUES_VIEW . $record->slug, 'method' => 'POST', 'name'=>'formQuiz ', 'novalidate'=>'')) !!}
<div class="col-md-12">
	...
	if ( $record->user_id > 0 ) {
		...
		if ( $user ) {
			echo getPhrase('name:') . '<a href="'.URL_USER_DETAILS.$user->slug.'">'.$record->full_name.'</a><br>';
			echo getPhrase('email:') . $record->email;
		} 
	} 
	?>
	<br>
	...
	?><br>
	{{getPhrase('date:' . $record->created_at)}}<br>
	{{getPhrase('browser:' . $record->user_agent)}}<br>
	{{'IP ' . getPhrase('address:' . $record->ip_address)}}
    </div> 
    <!-- Submit Form Input -->
	@if( $record->user_id > 0 )
		<!-- Message Form Input -->
