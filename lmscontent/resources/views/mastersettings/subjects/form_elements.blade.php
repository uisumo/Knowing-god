					...
					<fieldset class="form-group col-md-6">						
						{{ Form::label('color_class', getphrase('color_class')) }}
						<span class="text-red">*</span>
						<?php
						$color_classes = array(
							'text-green' => getPhrase( 'Green' ),
							'text-blue' => getPhrase( 'Blue' ),
							'text-yellow' => getPhrase( 'Yellow' ),
						);
						?>
						{{Form::select('color_class', $color_classes, null, ['class'=>'form-control','id'=>'color_class',
							'ng-model'=>'color_class',
							'required'=> 'true', 
							'ng-class'=>'{"has-error": formTopics.color_class.$touched && formTopics.color_class.$invalid}'
						])}}
						<div class="validation-error" ng-messages="formSubjects.color_class.$error" >
	    					{!! getValidationMessage()!!}
						</div>
					</fieldset>
					</div>
					...
					<fieldset class="form-group col-md-6">						
						{{ Form::label('placement_on_dashboard', getphrase('placement_on_dashboard')) }}
						<span class="text-red">*</span>
						<?php
						$placement_on_dashboard = array(
							'top-icnset' => getPhrase( 'Top' ),
							'right-icnset' => getPhrase( 'Right' ),
							'left-icnset' => getPhrase( 'Left' ),
						);
						?>
						{{Form::select('placement_on_dashboard', $placement_on_dashboard, null, ['class'=>'form-control','id'=>'placement_on_dashboard',
							'ng-model'=>'placement_on_dashboard',
							'required'=> 'true', 
							'ng-class'=>'{"has-error": formTopics.placement_on_dashboard.$touched && formTopics.placement_on_dashboard.$invalid}'
						])}}
						<div class="validation-error" ng-messages="formSubjects.placement_on_dashboard.$error" >
	    					{!! getValidationMessage()!!}
						</div>
					</fieldset>
