				...
				<div class="row">
					<div class="col-lg-12">
						<ol class="breadcrumb">
							...
							<li><a href="{{URL_ADMIN_DASHBOARDS}}">{{ getPhrase('dashboards')}}</a> </li>
				...
				<div class="panel panel-custom">
					<div class="panel-heading">
						<div class="pull-right messages-buttons">
							<a href="{{URL_ADMIN_DASHBOARDS}}" class="btn  btn-primary button" >{{ getPhrase('list')}}</a>
						</div>
					<h1>{{ $title }}  </h1>
					</div>
					...
					@if ($record)
					 <?php $button_name = getPhrase('update'); ?>
						{{ Form::model($record, 
						array('url' => URL_ADMIN_DASHBOARDS_EDIT.'/'. $record->slug, 
						'method'=>'patch', 'name'=>'formSubjects ', 'novalidate'=>'')) }}
					@else
						{!! Form::open(array('url' => URL_ADMIN_DASHBOARDS_ADD, 'method' => 'POST', 'name'=>'formSubjects ', 'novalidate'=>'')) !!}
					@endif

					 @include('mastersettings.dashboard-promotion.form_elements', 
