...
class Payment extends Model
{
    ...
    public static function isItemPurchased($item_id, $item_type = 'combo', $user_id='')
    {
		...
		/**
		 * Even the item is paid item, if admin given special permission to access this item to a special role user, its free!!
		 */
		if ( 'lms' === $item_type ) {
			$item_details = App\LmsSeries::where('id','=',$item_id)->first();
			if ( ! empty( $item_details ) ) {
				$special_role = getSpecialRole( $user_id );
				$free_for = $item_details->free_for;
				if ( ! empty( $special_role ) && ! empty( $free_for ) ) {
					if ( in_array( $special_role, explode( ',', $free_for ) ) ) {
						return TRUE;
					}					
				}
			}
        ...
        $subscription_records = Payment::where('start_date','<=',$date)
                          ...
                          ->get();
						  */
		$subscription_records = Payment::where('user_id','=',$user_id)
