...
class SettingsController extends Controller
{
  ...
  public function promotionCriteria()
	{
		$data['title'] = getPhrase( 'dashboard_promotion' );
		$data['dashboards'] = DB::table( 'dashboards' )->get();
		$data['active_class']       = 'dashboards';
		return view('mastersettings.dashboard-promotion.list', $data);
	}
	...
	public function addDashboard()
	{
		if(!checkRole(getUserGrade(2)))
      {
    	...
    	$data['record']         	= FALSE;
    	$data['active_class']       = 'master_settings';
    	$data['title']              = getPhrase('add_dashboard');
    	return view('mastersettings.dashboard-promotion.add-edit', $data);
	}
	...
	public function editDashboard( $slug )
	{
		if(!checkRole(getUserGrade(2)))
      {
    	...
    	$data['record']         	= FALSE;
    	$data['active_class']       = 'master_settings';
		$data['record']       = DB::table( 'dashboards' )->where( 'slug', '=', $slug )->first();
    	$data['title']              = getPhrase('edit_dashboard_settings');
    	return view('mastersettings.dashboard-promotion.add-edit', $data);
	}
	
	public function updateDashboard( Request $request, $slug )
	{
		if(!checkRole(getUserGrade(2)))
		{
		prepareBlockUserMessage();
		...
		// $record = DB::table( 'dashboards' )->where('slug', $slug)->get()->first();
		...
		try{			
			...
			$record['courses_criteria'] = $request->courses_criteria;
			$record['courses_from']		= $request->courses_from;
			$record['courses_to'] 		= $request->courses_to;
			
			$record['modules_criteria'] 		= $request->modules_criteria;
			$record['modules_from'] 		= $request->modules_from;
			$record['modules_to']		= $request->modules_to;
			
			$record['lessons_criteria'] 		= $request->lessons_criteria;
			$record['lessons_from'] 		= $request->lessons_from;
			$record['lessons_to'] 		= $request->lessons_to;
			
			$record['star_symbol']    = $request->star_symbol;
			$record['pathway_symbol'] = $request->pathway_symbol;
			$record['crown_symbol'] = $request->crown_symbol;
			
			DB::table( 'dashboards' )->where( 'slug', '=', $slug)->update( $record );
		 ...
		 catch(Exception $e)
		 {
		  ...
		  DB::rollBack();
		   if(getSetting('show_foreign_key_constraint','module'))
		   {
			  ...
			  flash('oops...!','improper_data_in_the_form', 'error');
		   }
		 }
		 return redirect(URL_ADMIN_DASHBOARDS);
	}
	
	public function showLMSMode()
	{
		if(!checkRole(getUserGrade(2)))
		{
			prepareBlockUserMessage();
		...
		$data['active_class']       = 'master_settings';
		$data['record']       = DB::table( 'lmsmode' )->first();
		$data['title']              = 'LMS ' . getPhrase( 'mode' );
		return view('mastersettings.lmsmode', $data);
	}
	
	public function updateLMSMode( Request $request )
	{
		if(!checkRole(getUserGrade(2)))
		{
			prepareBlockUserMessage();
		...
		$record	= array(
			'lmsmode' => $request->lmsmode,
		);		
		DB::table( 'lmsmode' )->update( $record );
		
		flash('success','record_updated_successfully', 'success');
		return redirect( URL_SHOW_LMSMODE );
	}
	...
	public function showLayout()
	{
		...
		$data['active_class']       = 'master_settings';
		$data['record']       = DB::table( 'lmsmode' )->first();
		$data['title']              = getPhrase( 'Layout' );
		return view('mastersettings.layout', $data);
	}
	...
	public function updateLayout( Request $request )
	{
		...
		flash('success','record_updated_successfully', 'success');
		return redirect( URL_SHOW_LAYOUT );
	}
