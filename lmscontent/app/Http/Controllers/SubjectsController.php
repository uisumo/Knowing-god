      ...
      if(!checkRole(getUserGrade(2)))
      {
        ...
        return back();
         ...
         $records = Subject::select([
         	'id','subject_title', 'subject_code','maximum_marks', 'pass_marks', 'is_lab', 'is_elective_type', 'slug', 'updated_at', 'color_class', 'placement_on_dashboard'])
		...
		->addColumn('color_class', function ( $records ) {
			return '<span class="' . $records->color_class . '">' . $records->color_class . ' (' . $records->placement_on_dashboard . ')</span>';
		})
        ->removeColumn('slug')
		...
		->removeColumn( 'placement_on_dashboard' )
        ->make();
    }
    ...
    public function update(Request $request, $slug)
    {
      if(!checkRole(getUserGrade(2)))
      {
        ...
        */
		...
		$record->color_class = $request->color_class;
		$record->placement_on_dashboard = $request->placement_on_dashboard;		
        $record->save();
