     ...
     public function processUpload(Request $request, $record, $file_name)
     {
        if(env('DEMO_MODE')) {
         ...
         if ($request->hasFile($file_name)) {
          ...
          $destinationPath      = public_path( $settings->categoryImagepath );
		 
          $fileName = $record->id.'_'.$file_name.'.'.$request->$file_name->guessClientExtension();
          
          $res = $request->file($file_name)->move($destinationPath, $fileName);
