...
class StudentQuizController extends Controller
{
    ...
    public function finishExam(Request $request, $slug)
    {
        ...
        if($this->examSession('check'))
        {
             ...
             $redirect_to = URL_FRONTEND_LMSCATEGORIES;
    ...
    public function examAnalysis($slug)
    {
      ...
      if(!isEligible($slug))
        ...
        $userid = $user->id;
      $data['active_class']       = 'analysis';
      $data['title']              = getPhrase('quiz_analysis_by_attempts');
      $data['user']               = $user;
        // Chart code start
        $records = array();
       ...
       $records = Quiz::join('quizresults', 'quizzes.id', '=', 'quizresults.quiz_id')
        ...
        $chart_data['type'] = 'pie'; 
        //horizontalBar, bar, polarArea, line, doughnut, pie
        $chart_data['title'] = getPhrase('quiz_analysis_by_exam'); 
        $border_color=[];
        $chart_data['data']   = (object) array(
            'labels'            => $labels,
            'dataset'           => $dataset,
    ...
    public function getExamAnalysisData($slug)
    {
        $user = User::getRecordWithSlug($slug);
         $records = array();
            ...
            $records = Quiz::join('quizresults', 'quizzes.id', '=', 'quizresults.quiz_id')
            ->select(['title','dueration', 'quizzes.total_marks',  \DB::raw('count(quizresults.user_id) as attempts, quizzes.slug, user_id') ])
            ->where('user_id', '=', $user->id)
            ->groupBy('quizresults.quiz_id')
            ->get();
    ...
    public function overallSubjectAnalysis($slug)
    {
      ...
      $data['active_class']       = 'analysis';
      $data['title']              = getPhrase('quiz_analysis_by_pathway');
      $data['user']               = $user;
      $userid = $user->id;
